/* global window document localStorage HTMLOptionElement */
...
class Nightmare extends Helper {
  ...
  async _init() {
    ...
    this.Nightmare.action('asyncScript', function (...args) {
      const done = args.pop();
      args = args.splice(1, 0, done);
      this.evaluate_now(...args);
    });
  ...
  _afterSuite() { // eslint-disable-line class-methods-use-this
  }
  ...
  _locate(locator) {
    locator = new Locator(locator, 'css');
    return this.browser.evaluate(
      (by, locator) => window.codeceptjs.findAndStoreElements(by, locator),
      locator.type,
      locator.value,
    );
  }
  ...
  async seeElement(locator) {
    locator = new Locator(locator, 'css');
    const num = await this.browser.evaluate((by, locator) => window.codeceptjs.findElements(by, locator).filter(e => e.offsetWidth > 0 && e.offsetHeight > 0).length, locator.type, locator.value);
  ...
  async dontSeeElement(locator) {
    ...
    const num = await this.browser.evaluate(
      (by, locator) => window.codeceptjs.findElements(by, locator).filter(e => e.offsetWidth > 0 && e.offsetHeight > 0).length,
      locator.type,
      locator.value,
    );
  ...
  async executeScript(...fn) {
    return this.browser.evaluate.apply(this.browser, ...fn)
   ...
   * {{> ../webapi/wait }}
   */
  async wait(sec) { // eslint-disable-line class-methods-use-this
  ...
  async waitForText(text, sec, context = null) {
    ...
    return this.browser.wait(
      (by, locator, text) => window.codeceptjs.findElement(by, locator).innerText.indexOf(text) > -1,
      locator.type,
      locator.value,
      text,
    ).catch((err) => {
  ...
  refresh() {
    // eslint-disable-next-line no-console
    console.log('Deprecated in favor of refreshPage'); // eslint-disable-line no-console
    return this.browser.refresh();
  }
...
async function proceedSee(assertType, text, context) {
  ...
  const texts = await this.browser.evaluate(
    (by, locator) => window.codeceptjs.findElements(by, locator).map(el => el.innerText),
    locator.type,
    locator.value,
  );
...
async function proceedSeeInField(assertType, field, value) {
  ...
  if (tag === 'select') {
    ...
    const text = await this.browser.evaluate(
      (el, val) => el.querySelector(`option[value="${val}"]`).innerText,
      el,
...
async function proceedIsChecked(assertType, option) {
  ...
  const selected = await this.browser.evaluate(
    els => els.map(el => window.codeceptjs.fetchElement(el).checked).reduce((prev, cur) => prev || cur),
    els,
  );
