...
class Nightmare extends Helper {
  ...
  async _init() {
    ...
    // Credit to: https://github.com/Zn4rK/nightmare-wait-for-url/blob/master/nightmare-wait-for-url.js
    ...
    this.Nightmare.action(
      ...
      function (matchUrl, strict = false, done) {
        const self = this;

        const timeout = setTimeout(async () => {
          const curUrl = await self.url();
          let message;
          if (strict) {
            message = `expected url to be ${matchUrl}, but found ${curUrl}`;
          } else {
            message = `expected url to include ${matchUrl}, but found ${curUrl}`;
          }
        ...
        const handler = (latestUrl) => {
          let isMatch = false;
          if (strict) {
            isMatch = latestUrl === matchUrl;
          } else {
            isMatch = latestUrl.indexOf(matchUrl) > -1;
          }
          if (isMatch) {
  ...
  async _before() {
    ...
    this.browser.options.waitTimeout = this.options.waitForTimeout || 1000; // Restore default timeout
    return this.browser;
  }
   ...
   *
   * ```js
   * this.helpers['Nightmare']._locateCheckable('I agree with terms and conditions').then // ...
   * ```
   */
  async _locateCheckable(locator) {
    return findCheckable.call(this, locator).then(res => res);
  }
   ...
   *
   * ```js
   * this.helpers['Nightmare']._locateClickable('Next page').then // ...
   * ```
   */
  async _locateClickable(locator) {
    return findClickable.call(this, locator).then(res => res);
  }
   ...
   *
   * ```js
   * this.helpers['Nightmare']._locateFields('Your email').then // ...
   * ```
   */
  async _locateFields(locator) {
    return findField.call(this, locator).then(res => res);
  }
   ...
   */
  async grabSource() {
    return this.browser.evaluate(() => document.documentElement.outerHTML);
  }
   ...
   *
   * Nightmare does **not** currently support any modifier keys (Control, Command, Alt, Shift etc.)
   *
   * ```js
   * I.presskey('hello');
  ...
  async pressKey(key) {
    ...
    if (Array.isArray(key)) {
      throw Error('Modifier keys are currently not supported by Nightmare');
    }
    ...
    if (isSpecialKey) {
      ...
      for (const char of key) {
        await this.browser.pressKey(char);
      }
    }
  ...
  async grabHTMLFrom(locator) {
    ...
    const html = await forEachAsync(els, (el) => {
      return this.browser.evaluate((el) => {
        return window.codeceptjs.fetchElement(el).innerHTML;
      }, el);
    });

    if (Array.isArray(html) && html.length === 1) {
      return html[0];
    }
    return html;
  }
  ...
  async seeAttributesOnElements(locator, attributes) {
    ...
    const missingAttributes = await forEachAsync(els, async (el) => {
      const attributeValues = await forEachAsync(attributeNames, async (attr) => {
        return this.browser.evaluate((el, attr) => {
          const match = window.codeceptjs.fetchElement(el);
          return match[attr] || match.getAttribute(attr);
        }, el, attr);
      });

      return attributeValues.filter((actual, i) => actual !== expectedValues[i]);
    });
  ...
  async grabCssPropertyFrom(locator, cssProperty) {
    ...
    assertElementExists(el, locator);
    return this.browser.evaluate((el, attr) => {
      const curEl = window.codeceptjs.fetchElement(el);
      const props = window.codeceptjs.getCssProperties(el, attr);
      return props[0];
    }, el, cssProperty);
  }
  ...
  async seeCssPropertiesOnElements(locator, cssProperties) {
    ...
    const cssPropertyNames = Object.keys(cssPropertiesCamelCase);
    const expectedValues = cssPropertyNames.map(name => cssPropertiesCamelCase[name]);

    const missingAttributes = await forEachAsync(els, async (el) => {
      const propValues = await this.browser.evaluate((el, attributes) => {
        return window.codeceptjs.getCssProperties(el, attributes);
      }, el, cssPropertyNames);

      // Normalise values
      let propObj = {};
      cssPropertyNames.forEach((prop, i) => propObj[prop] = propValues[i]);
      propObj = convertCssPropertiesToCamelCase(propObj);
      const normalizePropValues = Object.values(propObj);

      return normalizePropValues.filter((actual, i) => actual !== expectedValues[i]);
    });
  ...
  async waitForValue(field, value, sec) {
    this.browser.options.waitTimeout = sec ? sec * 1000 : this.options.waitForTimeout;
    const locator = new Locator(locator, 'css');

    return this.browser.wait((by, locator, expectedValue) => {
      const els = window.codeceptjs.findElements(by, locator);
      if (!els) return false;

      els.filter(el => `${el.value}`.indexOf(expectedValue)).length > 0;
  ...
  async waitNumberOfVisibleElements(locator, num, sec) {
    ...
    return this.browser.wait((by, locator, expectedTotal) => {
      const els = window.codeceptjs.findElements(by, locator);
      if (!els) return false;
      return els.filter(el => el.offsetWidth > 0 && el.offsetHeight > 0).length === expectedTotal;
    }, locator.type, locator.value, num).catch((err) => {
      if (err.message && err.message.indexOf('.wait() timed out after') > -1) {
        throw new Error(`The number of elements (${locator.value}) is not ${num} after ${sec} sec`);
      }
      throw err;
    });
  }
  ...
  async waitForEnabled(locator, sec) {
    ...
    return this.browser.wait((by, locator) => {
      ...
      if (err.message && err.message.indexOf('.wait() timed out after') > -1) {
        throw new Error(`element (${JSON.stringify(locator)}) still not enabled after ${sec} sec`);
      }
      throw err;
    });
  }
   ...
   * {{> ../webapi/waitUntil }}
   */
  async waitUntil(fn, sec = 0, timeoutMsg = '') {
    this.browser.options.waitTimeout = sec ? sec * 1000 : this.options.waitForTimeout;
    return this.browser.wait(fn).catch((err) => {
      if (err.message && err.message.indexOf('.wait() timed out after') > -1) {
        if (timeoutMsg) {
   ...
   * {{> ../webapi/waitInUrl }}
   */
  async waitInUrl(urlPart, sec) {
    this.browser.options.waitTimeout = sec ? sec * 1000 : this.options.waitForTimeout;
    return this.browser.waitForUrl(urlPart, false);
  }

  /**
   * {{> ../webapi/waitUrlEquals }}
   */
  async waitUrlEquals(urlPart, sec) {
    this.browser.options.waitTimeout = sec ? sec * 1000 : this.options.waitForTimeout;
    ...
    return this.browser.waitForUrl(urlPart, true);
  }
...
async function proceedSee(assertType, text, context, strict = false) {
  ...
  if (strict) {
    return texts.map(elText => equals(description)[assertType](text, allText));
  }
...
async function findClickable(locator, context) {
  ...
  if (matchedLocator.isAccessibilityId()) {
    return this.browser.findElement({ css: withAccessiblitiyLocator(matchedLocator.value) }, contextEl);
  }
  ...
  els = await this.browser.findElements({ xpath: Locator.clickable.self(literal) }, contextEl);
  if (els.length) {
    return els[0];
  }
