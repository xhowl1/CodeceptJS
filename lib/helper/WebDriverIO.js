...
class WebDriverIO extends Helper {
  ...
  async seeCssPropertiesOnElements(locator, cssProperties) {
    ...
    let props = await forEachAsync(res.value, async el => forEachAsync(
      Object.keys(cssProperties),
      async prop => this.browser.elementIdCssProperty(el.ELEMENT, prop),
    ));
  ...
  async seeAttributesOnElements(locator, attributes) {
    ...
    let attrs = await forEachAsync(res.value, async el => forEachAsync(
      Object.keys(attributes),
      async attr => this.browser.elementIdAttribute(el.ELEMENT, attr),
    ));
   ...
   */
  executeScript(...fn) {
    return this.browser.execute.apply(this.browser, ...fn).then(res => res.value);
  }
  ...
  executeAsyncScript(...fn) {
    return this.browser.executeAsync.apply(this.browser, ...fn).then(res => res.value);
  }
   ...
   */
  runOnIOS(caps, fn) { // eslint-disable-line no-unused-vars,class-methods-use-this
  }

  /**
   * placeholder for ~ locator only test case write once run on both Appium and WebDriverIO
   */
  runOnAndroid(caps, fn) { // eslint-disable-line no-unused-vars,class-methods-use-this
  }

  /**
   * placeholder for ~ locator only test case write once run on both Appium and WebDriverIO
   */
  runInWeb(fn) { // eslint-disable-line class-methods-use-this
    return fn();
  }
}
...
async function forEachAsync(array, callback, option = {}) {
  ...
  if (unifyResults) {
    return unify(values, { extractValue });
  }
  return values;
}
...
async function proceedSeeField(assertType, field, value) {
  ...
  const proceedMultiple = async (fields) => {
    const fieldResults = toArray(await forEachAsync(fields, async el => this.browser.elementIdAttribute(el.ELEMENT, 'value')));
  ...
  const filterSelectedByValue = async (elements, value) => filterAsync(elements, async (el) => {
