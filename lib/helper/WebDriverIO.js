...
class WebDriverIO extends Helper {
  ...
  async grabHTMLFrom(locator) {
    return this.browser.getHTML(withStrictLocator.call(this, locator), false).then(html => html);
  }
  ...
  async waitForText(text, sec = null, aContext = null) {
    ...
    return this.browser.waitUntil(
      ...
      `Text "${text}" was not found on page after ${aSec} sec`,
    );
  }
  ...
  async waitUntil(fn, sec = null, timeoutMsg = '') {
    const aSec = sec || this.options.waitForTimeout;
    return this.browser.waitUntil(fn, aSec, timeoutMsg).catch((err) => {
      if (err.message && err.message.indexOf('Promise was rejected with the following reason:') > -1) {
        if (timeoutMsg) {
...
async function proceedSee(assertType, text, context, strict = false) {
  ...
  if (!context) {
    ...
    const contextLocator = new Locator(context, 'css');
    description = `element ${contextLocator.toString()}`;
  }
  ...
  const allText = Array.isArray(selected) ? selected.join(' | ') : selected;
