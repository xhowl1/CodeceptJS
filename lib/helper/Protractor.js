...
class Protractor extends Helper {
  ...
  async click(locator, context = null) {
    ...
    if (context) {
      ...
      assertElementExists(els, context, 'Clickable element', `was not found inside element ${new Locator(context).toString()}`);
      matcher = els[0];
    }
  ...
  async switchTo(locator) {
    ...
    return this.browser
      ...
      .frame(els[0])
      .catch((e) => {
        if (/no such frame/i.test(e.message)) {
  ...
  async waitUntil(fn, sec = null, timeoutMsg = '') {
    ...
    return this.browser.wait(fn, waitTimeout, timeoutMsg);
  }
  ...
  async waitForText(text, sec = null, context = null) {
    ...
    return this.browser.wait(EC.textToBePresentInElement(el, text), aSec * 1000)
      .catch((e) => {
        if (/wait timed out after/i.test(e.message)) {
          throw new Error(`Text "${text}" was not found on page after ${aSec} sec\n${e.message}`);
        }
        throw e;
      });
  }
...
async function proceedSee(assertType, text, context, strict = false) {
  ...
  const promises = [];
  const texts = [];
  els.forEach(el => promises.push(el.getText().then(elText => texts.push(elText))));
...
async function findClickable(matcher, locator) {
  ...
  if (locator.isAccessibilityId()) {
    const els = await matcher.findElements(global.by.css(withAccessiblitiyLocator(locator.value)));
    if (els.length) {
      return els[0];
    }
  }
  ...
  els = await matcher.findElements(global.by.xpath(Locator.clickable.self(literal)));
  if (els.length) {
    return els[0];
  }

  return matcher.findElements(global.by.css(locator.value));
}
