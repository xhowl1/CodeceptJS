...
class Puppeteer extends Helper {
  constructor(config) {
    ...
    this.popupStore = new Popup();
    this.consoleLogStore = new Console();

    // override defaults with config
    Object.assign(this.options, config);
    this.puppeteerOptions = Object.assign({ headless: !this.options.show }, this.options.chrome);
    this.popupStore.defaultAction = this.options.defaultPopupAction;
  }
   ...
   */
  amAcceptingPopups() {
    this.popupStore.actionType = 'accept';
  }
   ...
   */
  acceptPopup() {
    this.popupStore.assertPopupActionType('accept');
  }
   ...
   */
  amCancellingPopups() {
    this.popupStore.actionType = 'cancel';
  }
   ...
   */
  cancelPopup() {
    this.popupStore.assertPopupActionType('cancel');
  }
  ...
  async seeInPopup(text) {
    this.popupStore.assertPopupVisible();
    const popupText = await this.popupStore.popup.message();
  ...
  _addPopupListener(page) {
    ...
    page.on('dialog', async (dialog) => {
      this.popupStore.popup = dialog;
      const action = this.popupStore.actionType || this.options.defaultPopupAction;
   ...
   */
  async grabPopupText() {
    if (this.popupStore.popup) {
      return this.popupStore.popup.message();
    }
    return null;
  }
  ...
  async _startBrowser() {
    ...
    const targetCreatedHandler = (page) => {
      ...
      page.on('load', () => {
        this.context = page.$('body');
      });
      page.on('console', (msg) => {
        this.debugSection(msg.type(), msg.args().join(' '));
        this.consoleLogStore.add(msg);
      });
    };
  ...
  async fillField(field, value) {
    ...
    if (tag === 'INPUT' || tag === 'TEXTAREA') {
      await this._evaluateHandeInContext((el) => {
        el.value = '';
      }, el);
    } else if (editable) {
      await this._evaluateHandeInContext((el) => {
  ...
  async selectOption(select, option) {
    ...
    for (const key in option) {
      ...
      if (optEl.length) {
        this._evaluateHandeInContext((el) => {
          el.selected = true;
        }, optEl[0]);
      } else {
  ...
  async grabBrowserLogs() {
    const logs = this.consoleLogStore.entries;
  ...
  async executeAsyncScript(...args) {
    const asyncFn = function (...fnArgs) {
      const fn = eval(fnArgs.shift()); // eslint-disable-line no-eval
