...
class Puppeteer extends Helper {
  ...
  async pressKey(key) {
    ...
    if (['Backspace', 'Delete'].indexOf(key) > -1) {
      await this.page.keyboard.press(key);
    } else if (key.length > 0) {
      if (['Shift', 'ShiftLeft', 'ShiftRight'].indexOf(modifier) > -1) {
        for (const currentKey of key) {
          if (/^\d$/i.test(currentKey)) {
            // Convert to Puppeteer Key Code so it can perform the conversion to
            // symbols (i.e. 1 -> !) [based on US keyboard]
            const keyCode = `Digit${currentKey}`;
            await this.page.keyboard.press(keyCode);
          } else {
            // Puppeteer only supports uppercase conversion for US Keyboard characters
            // so perform the conversion manually to support chars like ä, ö, ü etc.
            await this.page.keyboard.type(currentKey.toUpperCase());
          }
        }
      } else {
  ...
  async switchTo(locator) {
    ...
    const tag = await els[0].getProperty('tagName').then(el => el.jsonValue());
   ...
   * {{> ../webapi/waitUntil }}
   */
  async waitUntil(fn, sec = null, timeoutMsg = '') {
    const aSec = sec || this.options.waitForTimeout;
    const waitTimeout = aSec * 1000;
    const context = await this._getContext();
    return context.waitForFunction(fn, { timeout: waitTimeout }).catch((err) => {
      if (err.message && err.message.indexOf('waiting failed:') > -1) {
        if (timeoutMsg) {
...
async function findClickable(matcher, locator) {
  ...
  if (locator.isAccessibilityId()) return findElements.call(this, matcher, withAccessiblitiyLocator(locator.value));
