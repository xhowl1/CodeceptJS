if (!window.codeceptjs) {
  ...
  codeceptjs.getCssProperties = function (el, properties) {
    const curEl = this.fetchElement(el);
    ...
    if (properties) {
      ...
      const values = props.map((prop) => {
        let value = style[prop] || style.getPropertyValue(prop);
        // Normalize the cursor auto value, as all other web drivers do this.
        // There could be more tag names to also consider
        const specialTags = ['A'];
        if (prop === 'cursor' && value === 'auto' && specialTags.indexOf(curEl.tagName) > -1) {
          value = 'pointer';
        }
        return value;
      });
  ...
  codeceptjs.hoverEl = function (el, x = 0, y = 0) {
    if (document.activeElement instanceof HTMLElement) {
      document.activeElement.blur();
    }

    const rect = this.fetchElement(el).getBoundingClientRect();
    ...
    if (x || y) {
      // Find element at new coordinates with offset (it could be some other element)
      const elAtPoint = document.elementFromPoint(middleX + x, middleY + y);
      if (elAtPoint) {
        elAtPoint.dispatchEvent(event);
      }
    } else {
      this.fetchElement(el).dispatchEvent(event);
    }
