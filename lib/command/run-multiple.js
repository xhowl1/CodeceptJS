...
const promiseAllIncludeErrors = (promises) => {
...
module.exports = function (suites, options) {
  ...
  runHook(config.multipleBootstrap, done, 'multiple.bootstrap');
  ...
  const forkPromises = suites.map((suite) => {
    ...
    const runSuitePromises = runSuite(suiteName, suiteConf);
    return Promise.all(runSuitePromises).then(() => {
      return false;
    });
  });

  return promiseAllIncludeErrors(forkPromises)
    .then((results) => {
    ...
    .then(() => {
      ...
      runHook(config.multipleTeardown, done, 'multiple.teardown');
