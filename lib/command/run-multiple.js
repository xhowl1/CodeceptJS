...
module.exports = function (suites, options) {
  ...
  if (config.multipleBootstrap) {
    const done = () => '';
    runHook(config.multipleBootstrap, done, 'bootstrap');
  }

  // iterate options
  const forkPromises = suites.map((suite) => {
    // get suites
    suite = suite.split(':');
    ...
    const cProm = runSuite(suiteName, suiteConf);
    console.log('cPromp: ', cProm);
    return Promise.all(cProm).then(() => {
      console.log('one prom finished');
      return true;
    });
  });

  console.log('promsises', forkPromises);
  return Promise_all(forkPromises)
    .then((results) => {
      console.log('all forks are finished');

      const okResults = results.filter(item => item.code === 0);
    ...
    .then(() => {
      ...
      if (config.multipleTeardown) {
        const done = () => '';
        console.log('calling multiple teardown');
        runHook(config.multipleTeardown, done, 'teardown');
      }
    });
};
...
function runSuite(suite, suiteConf, browser) {
  if (!browser) {
    // run configurations
    const results = suiteConf.browsers.map((b) => {
